<template>
  <div id="shpDiv">
    ...
    <el-card class="airSel">
      <el-select
        v-model="filterPropName"
        placeholder="请选择"
        @change="Query(filterPropName)"
      >
        <el-option
  ...
  methods: {
    ...
    getWFS() {
      ...
      let wfsVectorSource = new VectorSource({
        ...
        url:
          "http://localhost:8080/geoserver/cite/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=cite%3AExport_Output&maxFeatures=50&outputFormat=text%2Fxml%3B%20subtype%3Dgml%2F2.1.2",
        strategy: bbox,
      });
